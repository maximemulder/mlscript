...
def combine(left: Type, right: Type, dir: Direction)(using ctx: Context, cache: VarCache): Type =
  dir match
...
extension (types: List[Type])
  ...
  def combineMany(dir: Direction)(using ctx: Context, cache: VarCache): Type =
    dir match
      case Direction.Sub =>
        types.meetMany()
