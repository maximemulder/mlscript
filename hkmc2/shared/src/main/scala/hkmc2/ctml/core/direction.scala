...
def combine(left: Type, right: Type, dir: Direction)(using ctx: Context) =
  dir match
...
def combineMany(types: List[Type], dir: Direction)(using ctx: Context) =
...
def getExtremalType(dir: Direction) =
  dir match
