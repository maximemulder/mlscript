...
extension (ctx: SubContext)
  ...
  def simplifyLevel(type1 : Type, level: Int, outs1: SubClauses)(using noInlineVars: NoInlineVars): (Type, SubClauses) =
    val (type2, outs2) = ctx.simplifyClauses2(type1, level, outs1)

    val type3 = type2.simplify()(using ctx.extend(outs2), noInlineVars)
    val outs3 = outs2
