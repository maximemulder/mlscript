...
extension (clauses: Clauses)
  /** Get the list of type variable that directly depend on another type variable in the clauses. */
...
extension (clause: Clause)
  /** Get the list of type variable that directly depend on another type variable in the clause. */
  def getDependentVars(varName: String): Set[String] =
    clause match
