...
class TypeSubstituteParams(val var_ : TypeVar, val substitute: TypeVar)

/** Implementation of the type substitution operation. */
object TypeSubstitute extends TypeDispatcher[Id, Id, TypeSubstituteParams](TypeIdentityCombinator[TypeSubstituteParams]):
